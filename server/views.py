...
excelDB_path = os.path.abspath(__file__).replace('views.py', 'static\excelData\data.xlsx')
...
@app.route('/detail_1',methods=['GET'])
@login_required
def detail():
    # 获取 GET 请求传递的数据
    taskid = request.args.get('taskid')

    excel_data = pd.read_excel(excelDB_path, sheet_name='Sheet1')
    # 将数据转换为字典列表
    data_list = excel_data.replace({pd.np.nan: ''}).to_dict(orient='records')
    taskdic = {}
    for data in data_list:
        if data["TaskId"] == int(taskid):
            if data['Type']:
                data['Type'] = [x for x in data['Type'].split(',')]
            else:
                data['Type'] = []
            taskdic["taskid"] = data["TaskId"]  # res.taskid
            taskdic["host"] = data["Host"]  # res.ip
            taskdic["state"] = data["State"]  # res.state
            taskdic["startime"] = data["StartTime"]  # res.startime
            taskdic["finishtime"] = data["FinishTime"]  # res.finishtime
            taskdic["type"] = data["Type"]  # res.text
            taskdic["detect"] = data["Detect"]
    # 将数据传递给 HTML 页面
    print(taskdic)
    return render_template('detail_1.html', taskdic=taskdic)
...
def showreport(id):
    ...
    obj1 =  newtask = TaskConfig(userid=current_user.id, ip=form.targetip.data, mode=0, startime=startime, state=0, text=form.comment.data)
    host = taskres.ip
...
def showtask():
    ...
    # 读取 Excel 文件
    # path = os.path.abspath(__file__).replace('views.py', 'data.xlsx')
    excel_data = pd.read_excel(excelDB_path, sheet_name='Sheet1')
    # 将数据转换为字典列表
    data_list = excel_data.replace({pd.np.nan: ''}).to_dict(orient='records')
    for data in data_list:
